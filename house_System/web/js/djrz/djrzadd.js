...
function init() {
	...
	//得到下拉框的值 
	$.ajax({
		url:'http://localhost:8081/house_System_Web_exploded/myCus',
		dataType:'json',
		type:'post',
		data:'',
		async : true,
	...
	$.ajax({
		url:'http://localhost:8081/house_System_Web_exploded/myArea',
		dataType:'json',
		type:'post',
		data:'',
		async : true,
		...
		if(aid!=0)
			{
					...
					$.ajax({
					url:'http://localhost:8081/house_System_Web_exploded/myHouse',
					dataType:'json',
					type:'post',
					data:{aid:aid},
					async : true,
...
function commitItem()
{
	...
	if(hid==0)
		...
		$.ajaxFileUpload({
	    url:'http://localhost:8081/house_System_Web_exploded/myDj/add',
